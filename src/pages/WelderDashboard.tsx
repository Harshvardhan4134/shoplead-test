...
const WelderDashboard = () => {
  ...
  // Filter welding jobs (simplified filter)
  const weldingJobs = jobs.filter(job => 
    (job.status === 'In Progress' || job.status === 'New') &&
    (!job.work_center || job.work_center === 'Welding' || job.work_center.toLowerCase().includes('weld'))
  );
  ...
  return (
    <div className="container mx-auto p-6">
      <div className="mb-6">
        <h1 className="text-2xl font-bold mb-1">Welder Dashboard</h1>
        <p className="text-gray-600">View and manage welding jobs</p>
      </div>
        ...
        <div className="bg-white p-4 rounded-lg shadow">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold">{weldingJobs.length}</p>
            </div>
        ...
        <div className="overflow-x-auto">
          ...
          {sortedJobs.length === 0 && (
            <div className="text-center py-8">
              <p className="text-gray-500">No welding jobs found matching your search criteria.</p>
            </div>
          )}
        </div>
      </div>
